...
class AllianzPipeline:

    # Database setup
    conn = sqlite3.connect('allianz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `allianz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM allianz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO allianz (date, title, link, content)"
